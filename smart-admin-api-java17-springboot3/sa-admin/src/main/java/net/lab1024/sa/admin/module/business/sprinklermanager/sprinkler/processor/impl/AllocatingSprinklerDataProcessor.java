...
import net.lab1024.sa.admin.module.business.sprinklermanager.sprinkler.domain.form.AllocatingSprinklerImportForm;
...
/**
 * 领用中喷头数据处理实现类
 * 处理导入的领用中喷头数据，进行数据校验、分仓存储及主表状态更新
 */
@Component("allocating") // 通过组件名称标识处理器类型
public class AllocatingSprinklerDataProcessor implements DataProcessor<AllocatingSprinklerImportForm> {
    ...
    @Resource
    private AllocatingSprinklerRepository allocatingSprinklerRepository; // 领用中数据访问层
     ...
     */
    @Override
    public ResponseDTO<String> process(List<AllocatingSprinklerImportForm> importVOs) {
        // 1. 空数据校验（基础校验优化）
        ...
        try{
            ...
            Map<Boolean, List<AllocatingSprinklerImportForm>> preprocessed = importVOs.stream()
                    .collect(Collectors.partitioningBy(
                            form -> StringUtils.isNotBlank(form.getSprinklerSerial())
                                    && form.getStatus().equals(RepositorySprinklerTypeChineseEnum.ALLOCATING_REPOSITORY.getDesc()))
                    );
            List<AllocatingSprinklerImportForm> validForms = preprocessed.get(true);
            List<AllocatingSprinklerImportForm> invalidForms = preprocessed.get(false);
            ...
            List<AllocatingSprinklerEntity> entities = filteredForms.stream()
            ...
            filteredForms.forEach(form -> {
                SprinklerEntity mainRecord = mainTableMap.get(form.getSprinklerSerial());
                if (mainRecord.getStatus() != RepositorySprinklerTypeEnum.ALLOCATING_REPOSITORY.getValue().byteValue()) {
                    mainRecord.setStatus(RepositorySprinklerTypeEnum.ALLOCATING_REPOSITORY.getValue().byteValue());
            ...
            if (!mainTableUpdates.isEmpty()) {
                ...
                updateWrapper.in("sprinkler_id", mainIdsToUpdate)
                        .set("status", RepositorySprinklerTypeEnum.ALLOCATING_REPOSITORY.getValue().byteValue());
                sprinklerRepository.update(updateWrapper);
            }
            // 8.2 批量插入领用中数据（使用MyBatis-Plus批量操作优化）
            if (!entities.isEmpty()) {
                try {
                    allocatingSprinklerRepository.saveBatch(entities);
     ...
     * @return 领用中实体
     */
    private AllocatingSprinklerEntity convertToWarehouseEntity(
            AllocatingSprinklerImportForm form,
            Map<String, SprinklerEntity> mainTableMap
    ) {
        // 使用Bean拷贝工具优化属性复制
        AllocatingSprinklerEntity entity = SmartBeanUtil.copy(form, AllocatingSprinklerEntity.class);
     ...
     * @return 无效序列号集合
     */
    private Set<String> collectInvalidSerials(List<AllocatingSprinklerImportForm> invalidForms) {
     ...
     * @return 已存在序列号集合
     */
    private Set<String> getExistingSprinklerSerials(List<AllocatingSprinklerImportForm> validForms) {
        ...
        // 使用单次查询优化数据库访问
        return allocatingSprinklerRepository.getBaseMapper()
                .selectList(new QueryWrapper<AllocatingSprinklerEntity>().in("sprinkler_serial", serialsToCheck))
                .stream()
                .map(AllocatingSprinklerEntity::getSprinklerSerial)
                .collect(Collectors.toSet());
    }
}