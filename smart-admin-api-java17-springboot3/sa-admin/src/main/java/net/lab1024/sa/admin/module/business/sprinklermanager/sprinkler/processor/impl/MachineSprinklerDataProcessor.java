...
public class MachineSprinklerDataProcessor implements DataProcessor<MachineSprinklerCreateForm> {
    ...
    public MachineSprinklerDataProcessor(SprinklerRepository sprinklerRepository) {
    ...
    public ResponseDTO<String> process(List<MachineSprinklerCreateForm> createVOs) {
        ...
        List<MachineSprinklerCreateForm> validForms = preprocessed.get(true);//有效数据
        List<MachineSprinklerCreateForm> invalidForms = preprocessed.get(false);//无效数据
        ...
        filteredForms.forEach(form->{
            SprinklerEntity mainRecord = mainTableMap.get(form.getSprinklerSerial());
            if(mainRecord.getStatus()!=1){
                mainRecord.setStatus((byte) 1);
    ...
    /**
     * 转换表单为机台实体（使用工具类进行属性拷贝）
     */
    private MachineSprinklerEntity convertToWarehouseEntity(MachineSprinklerCreateForm form) {
        return SmartBeanUtil.copy(form, MachineSprinklerEntity.class);
    }

    /**
     * 批量查询已存在的机台序列号（优化关键点：将N次查询合并为1次）
     */
